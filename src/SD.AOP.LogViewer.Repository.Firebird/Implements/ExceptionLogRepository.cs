...
namespace SD.AOP.LogViewer.Repository.Firebird.Implements
{
    /// <summary>
    /// 异常日志仓储Firebird实现
    /// </summary>
    public class ExceptionLogRepository : IExceptionLogRepository
    {
        ...
        {
            //初始化SQL工具
            _SqlHelper = new FbSqlHelper(GlobalSetting.DefaultConnectionString);
        }
        ...
        public ExceptionLog Single(Guid id)
        {
            ...
            using (IDataReader reader = _SqlHelper.ExecuteReader(sql, new FbParameter("@Id", id)))
            {
                if (reader.Read())
                {
        ...
        public ICollection<ExceptionLog> GetExceptionLogs(Guid? logId, DateTime? startTime, DateTime? endTime, int pageIndex, int pageSize, out int pageCount, out int rowCount)
        {
            ...
            //分页处理
            sql = $"{sql} ORDER BY \"OccurredTime\" DESC OFFSET {(pageIndex - 1) * pageSize} ROWS FETCH NEXT {pageSize} ROWS ; ";
