...
//加载用户列表
function getExceptionLogs(queryParams) {

    //指定url
    $.easyuiExt.datagrid.url = "/ExceptionLog/GetExceptionLogs";
    ...
    $.easyuiExt.datagrid.columns = [
        [
            ...
            { field: "OccurredTime", title: "发生时间", align: "center", halign: "center" },
...
function searchExceptionLogs() {
    ...
    getExceptionLogs(queryParams);
}

//查看日志
function watchLog(loginId) {
    $.easyuiExt.showWindow("查看日志", "/ExceptionLog/Detail/" + loginId, 1024, 600);
}
...
function removeLog(loginId) {
    $.easyuiExt.messager.confirm("Warning", "确定要删除吗？", function (confirm) {
        if (confirm) {
            $.ajax({
                type: "post",
                url: "/ExceptionLog/RemoveLog/" + loginId,
                success: function () {
