    ...
    public class RunningLogRepository
    {
        ...
        public RunningLog Single(Guid id)
        {
            ...
            using (SqlDataReader reader = _SqlHelper.ExecuteReader(sql, new SqlParameter("@Id", id)))
            {
                ...
                throw new NullReferenceException(string.Format("Id为\"{0}\"的运行日志不存在！", id));
            }
        }
        #endregion
        ...
        private RunningLog GetEntity(SqlDataReader reader)
        {
            RunningLog runningLog = new RunningLog
            {
                ...
                UserId = (Guid)Common.ToNetValue(reader, "UserId"),
                UserName = (string)Common.ToNetValue(reader, "UserName"),
                StartTime = (DateTime)Common.ToNetValue(reader, "StartTime"),
                StartTimeString = Convert.ToDateTime(Common.ToNetValue(reader, "StartTime")).ToString("yyyy-MM-dd HH:mm:ss.fff"),
                EndTime = (DateTime)Common.ToNetValue(reader, "EndTime"),
                IPAddress = (string)Common.ToNetValue(reader, "IPAddress"),
                EndTimeString = Convert.ToDateTime(Common.ToNetValue(reader, "EndTime")).ToString("yyyy-MM-dd HH:mm:ss.fff")
            };
            return runningLog;
        }
        #endregion
